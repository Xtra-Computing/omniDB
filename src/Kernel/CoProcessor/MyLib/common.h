...
//pivots 
void getPivot(Record *Rin, int Query_rLen, int sampleLevel, cmp_func fcn, Record** sampleRout);
...
void printString(char *data, Record* d_R, int Query_rLen);

//two producdure for validation
void validateSort(Record *R, int Query_rLen);
void validateSplit(Record *R, int Query_rLen, int numPart);
